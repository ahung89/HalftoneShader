    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				//(1 / _NumLevels))) * (1 / _NumLevels)
				float inc = 1 / _NumLevels;

				return lerp(fixed4(1, 1, 1, 1) * dotCol, floor(Luminance(pow((texCol.rgb), _LuminancePower)) / inc) * inc, _LuminanceLerp);
            }
            ENDCG
        }
    }
